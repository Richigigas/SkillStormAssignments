...
SELECT 
	...
	u.user_dob,
	u.user_email_id,
	DATENAME(WEEKDAY, u.user_dob) AS [user_day_of_birth]
FROM users u
WHERE MONTH(u.user_dob) = 5
ORDER BY DAY(u.user_dob);
GO

-- Exercise 3
SELECT
	u.user_id,
	UPPER(CONCAT_WS(' ', u.user_first_name, u.user_last_name)) AS [user_name],
	u.user_email_id,
	u.created_ts,
	YEAR(created_ts) AS [created_year]
FROM users u
WHERE YEAR(u.created_ts) = 2019
ORDER BY user_name;
GO
